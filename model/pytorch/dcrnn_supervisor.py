...
from model.pytorch.loss import simultaneous_quantile_loss
...
class DCRNNSupervisor:
    ...
    def save_model(self, epoch):
        ...
        config['epoch'] = epoch
        torch.save(config, self.path +'/models/epo%d.tar' % epoch)
        self._logger.info("Saved model at {}".format(epoch))
        return self.path + '/models/epo%d.tar' % epoch

    def load_model(self):
        self._setup_graph()
        assert os.path.exists(self.path + '/models/epo%d.tar' % self._epoch_num), 'Weights at epoch %d not found' % self._epoch_num
        checkpoint = torch.load(self.path + '/models/epo%d.tar' % self._epoch_num, map_location='cpu')
    ...
    def evaluate(self, dataset='val', batches_seen=0):
        ...
        with torch.no_grad():
            ...
            for _, (x, y) in enumerate(val_iterator):
                ...
                loss = self._compute_loss(y, output, quantiles)
            ...
            return mean_loss, y_preds_scaled, y_truths_scaled
    ...
    def _train(self, base_lr,
        ...
        for epoch_num in range(self._epoch_num, epochs):
            ...
            for _, (x, y) in enumerate(train_iterator):
                ...
                quantiles = [np.random.uniform(0, 0.5), np.random.uniform(0, 1), np.random.uniform(0.5, 1)]
                #quantiles = [0.025, 0.5, 0.975]
                loss = self._compute_loss(y, output, quantiles)
            ...
            val_loss, _, _ = self.evaluate(dataset='val', batches_seen=batches_seen)
    ...
    def _compute_loss(self, y_true, y_predicted, quantiles):
        y_true = self.standard_scaler.inverse_transform(y_true)
        y_predicted = self.standard_scaler.inverse_transform(y_predicted)
        return simultaneous_quantile_loss(y_predicted, y_true, quantiles)
